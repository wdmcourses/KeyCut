...
// segments: array of [start, end] kept ranges (seconds).
async function startExport({ src, segments, out, tmpDir, ffmpeg, onProgress }) {
  const started = fs.existsSync(tmpDir);
  ...
  try {
    const partNames = [];
    // MPEG-TS only muxes mp4-family codecs properly (AV1/VP9 land as bin_data).
    // For everything else use Matroska parts, which carry any codec - this is
    // what makes the splicer eat webm/mkv/etc. sources losslessly.
    ...
    // concat list with forward slashes
    ...
    // Always clean tmp/ - remove files and folder even if we just created it.
    try {
