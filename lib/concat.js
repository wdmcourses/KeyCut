...
function concatCancelled() {
  return _concatCancelled;
}

function resetConcatCancelled() {
...
function runFfmpeg(ffmpeg, args, onProgress, totalDuration) {
  return new Promise((resolve, reject) => {
    ...
    _concatChild = child;
    let stderr = '';
    const timer = setTimeout(() => { try { child.kill('SIGKILL'); } catch {} reject(new Error('ffmpeg timed out')); }, CONCAT_TIMEOUT_MS);
    ...
    child.on('error', (err) => { clearTimeout(timer); _concatChild = null; reject(err); });
    child.on('close', (code) => {
      clearTimeout(timer);
      _concatChild = null;
...
async function joinFiles({ files, ffmpeg, onProgress, totalDuration }) {
  ...
  const emit = (progress, index, total) => { if (onProgress) onProgress({ progress, index, total }); };
  try {
    emit(0.01, 0, files.length);
    const infos = [];
    let hasAudio = false;
    let audioRate = null;
    let audioCh = null;
    ...
    if (direct && !needSilent) {
      ...
      await runFfmpegList(ffmpeg, [
        ...
        '-c', 'copy',
        out
      ], listStr, onProgress ? (p) => emit(p, files.length - 1, files.length) : null, totalDuration || 0);
      emit(1, files.length - 1, files.length);
      return { ok: true, out };
    }
    ...
    for (let i = 0; i < infos.length; i++) {
      ...
      await runFfmpeg(ffmpeg, args, onProgress ? (p) => emit(0.02 + (i / files.length) * 0.7 + p * (0.7 / files.length), i, files.length) : null, segDur);
    }
    if (_concatCancelled) throw new Error('Cancelled');
    ...
    await runFfmpegList(ffmpeg, [
      ...
      '-c', 'copy',
      out
    ], listStr, onProgress ? (p) => emit(0.72 + p * 0.28, files.length - 1, files.length) : null, totalDuration || 0);
    emit(1, files.length - 1, files.length);
    return { ok: true, out };
  } catch (err) {
...
module.exports = { joinFiles, killConcat, concatCancelled, resetConcatCancelled, outputNameFor };