...
// Fast universal probe via ffmpeg STREAM COPY + framecrc: frames are listed
// without decoding, so keyframes come back in milliseconds even for 4K/long
// files. Works for every container/codec ffmpeg demuxes (mkv, webm, avi, ts,
// flv, wmv, ...). Contrast with the old select(pict_type)+showinfo approach,
// which decoded the whole file and hung on large inputs ("Analyzing…").
...
function parseFfmpegOutput(stderr, stdout) {
  ...
  // Stream timebase: "#tb 0: 1/1000" (per-stream). Converts pts -> seconds.
  let tbN = 1000;
  const tb = stdout.match(/#tb 0: 1\/(\d+)/);
  if (tb) tbN = parseInt(tb[1], 10);

  // framecrc lines: "stream, dts, pts, dur, size, crc[, F=0x0]". Keyframes have
  // no F= flag.
  const seen = new Set();
    ...
    try {
      ...
      if (parsed && parsed.duration > 0) {
        ...
        if (!keyTimes || keyTimes.length === 0) {
          // no stss / empty stss => every sample is a sync sample; stride to keep it sane
