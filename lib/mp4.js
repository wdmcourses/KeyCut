...
async function parseTkhdMatrix(reader, trakChildren) {
  const tkhd = trakChildren.find((c) => c.type === 'tkhd');
  if (!tkhd) return null;
  const head = await reader.read(tkhd.dataStart, 4);
  const version = head.readUInt8(0);
  const matrixOffset = tkhd.dataStart + (version === 1 ? 52 : 40);
  const m = await reader.read(matrixOffset, 36);
  return {
    a: m.readInt32BE(0),
    b: m.readInt32BE(4),
    u: m.readInt32BE(12),
...
function displayRotation(matrix) {
  if (!matrix) return 0;
  const ONE = 65536;
  const E = Math.round(ONE * 0.05);
  const { a, b, u, d } = matrix;
  if (Math.abs(Math.abs(a) - ONE) <= E && Math.abs(Math.abs(d) - ONE) <= E && Math.abs(b) <= E && Math.abs(u) <= E) return 0;
  if (Math.abs(a) <= E && Math.abs(d) <= E && Math.abs(Math.abs(b) - ONE) <= E && Math.abs(Math.abs(u) - ONE) <= E && Math.abs(b + u) <= E) return b > 0 ? 90 : 270;
  if (Math.abs(a + ONE) <= E && Math.abs(d + ONE) <= E && Math.abs(b) <= E && Math.abs(u) <= E) return 180;
  return 0;
}
...
async function parseMoov(reader, moovBox) {
  ...
  for (const trak of traks) {
    ...
    const rotation = displayRotation(await parseTkhdMatrix(reader, trakChildren));
    if (size && (rotation === 90 || rotation === 270)) {
      const t = size.width;
