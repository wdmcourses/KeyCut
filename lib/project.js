...
async function loadProject(filePath) {
  return new Promise((resolve, reject) => {
    const result = { src: null, srcRel: null, srcAbs: null, video: null, keys: [], cuts: [], deleted: [], cursor: 0, zoom: 0, viewStart: 0, markers: [] };
    let leftover = '';
    let sawSrc = false;

    const re = /<src\s+([^>]*?)\/?>(?:<\/src>)?|<src>(.*?)<\/src>|<video\s+([^>]*?)\/>|<view\s+([^>]*?)\/>|<k\s+t="([0-9.]+)"\/>|<c\s+t="([0-9.]+)"(?:\s+d="([01])")?\/>|<m\s+([^>]*?)\/>/g;
