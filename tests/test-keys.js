...
app.whenReady().then(async () => {
  ...
  // [2] click selects segment
  console.log('[2] click selects segment');
  await clickAt(k2, 48); // segments row (y 40..56)
  ...
  await sendCtrl('KeyZ', 'z'); // undo R -> deleted [false,true]
  ...
  await sendCtrl('KeyZ', 'z'); // undo X -> deleted [false,false]
  ...
  await sendCtrl('KeyZ', 'z', true); // redo -> deleted [false,true]
  ...
  // [6] S / F keyframe navigation (kept area only; [k1,10] is dimmed)
  ...
  // [7] playback skips dimmed segments
  console.log('[7] dimmed segments skipped in playback');
  await js(`window.__app.model.reset(${DUR}); window.__app.model.snapshot();`);
  await js(`window.__app.timeline.selectedIndex = null; window.__app.seekTo(${k1}); window.__app.cut();`); // cuts [0,k1,10]
  await js(`window.__app.timeline.selectedIndex = 1; window.__app.deleteSegment();`); // dim [k1,10]
  ...
  // [9] gray (dimmed) area behaves as one snap unit
  ...
  await js(`window.__app.seekTo(${(k1 + k2) / 2}); window.__app.deleteSegment();`); // dim [k1,k2]
  ...
  // [11] playback guard skips gray runs precisely (blue->gray->blue)
  console.log('[11] playback guard');
  await js(`window.__app.model.reset(${DUR}); window.__app.model.snapshot();`);
  await js(`window.__app.timeline.selectedIndex = null; window.__app.seekTo(${k1}); window.__app.cut(); window.__app.seekTo(${k2}); window.__app.cut();`);
  await js(`window.__app.timeline.selectedIndex = 1; window.__app.deleteSegment();`); // gray [k1,k2]
  const mid = (k1 + k2) / 2;
  ...
  await js(`window.__app.timeline.selectedIndex = 1; window.__app.deleteSegment();`); // gray [k1,10]
  ...
  // [12] shift+click multi-select + U merge
  ...
  // gray in the middle blocks selection extension and merge
  await js(`window.__app.model.reset(${DUR}); window.__app.model.snapshot();`);
  await js(`window.__app.timeline.selectedIndex = null; window.__app.seekTo(${k1}); window.__app.cut(); window.__app.seekTo(${k2}); window.__app.cut(); window.__app.seekTo(${k3}); window.__app.cut();`);
  await js(`window.__app.timeline.selectedIndex = 1; window.__app.deleteSegment();`); // gray segment 1 [k1,k2]
  await clickAt(k1 / 2, 48); // anchor segment 0
  await clickAt((k2 + k3) / 2, 48, true); // shift+click segment 2 (across gray)
  ...
  // [13] smooth caret during playback + C cuts at nearest keyframe
  ...
  const smoothT = k1 + 0.3; // between keyframes
  ...
  // [14] drag kept|gray boundary to extend the blue segment
  ...
  await js(`window.__app.timeline.selectedIndex = 1; window.__app.deleteSegment();`); // gray [k1,k2], kept [0,k1],[k2,10]
  // partial extend right (boundary k1 -> interior key)
  const part = keys[7]; // between k1 and k2
  ...
  // full consume right (boundary -> k2)
  ...
  // extend left (kept [k2,10], gray [0,k2] after reverse build)
  await js(`window.__app.model.reset(${DUR}); window.__app.model.snapshot(); window.__app.refreshActiveKeys();`);
  await js(`window.__app.seekTo(${k1}); window.__app.cut(); window.__app.seekTo(${k2}); window.__app.cut();`);
  await js(`window.__app.timeline.selectedIndex = 0; window.__app.deleteSegment();`); // gray [0,k1], kept [k1,k2],[k2,10]
  ...
  // [15] shrink blue by dragging the boundary into it
  console.log('[15] shrink blue into gray');
  await js(`window.__app.model.reset(${DUR}); window.__app.model.snapshot(); window.__app.refreshActiveKeys();`);
  await js(`window.__app.seekTo(${k1}); window.__app.cut(); window.__app.seekTo(${k2}); window.__app.cut();`);
  await js(`window.__app.timeline.selectedIndex = 1; window.__app.deleteSegment();`); // gray [k1,k2], blue [0,k1],[k2,10]
  const shrinkKey = keys[3]; // inside blue [0,k1]
  ...
  // consume the blue segment entirely (drag boundary to 0)
  ...
  // [16] starting a mouse scrub stops playback
  ...
  await clickAt(k1 / 2, 48); // mousedown on the timeline
  const pausedAfter = await js(`window.__app.video.paused`);
  check('mousedown on timeline pauses playback', pausedAfter);

  // [17] skipTo mutes during the seek (no gray audio leak) and restores after
  ...
  // [18] gray blocks can be selected and merged too
  ...
  await js(`window.__app.timeline.selectedIndex = 1; window.__app.deleteSegment(); window.__app.timeline.selectedIndex = 2; window.__app.deleteSegment();`); // gray [k1,k2],[k2,k3]
  // shift+click within the gray run
  await clickAt((k1 + k2) / 2, 48); // anchor gray segment 1
  await clickAt((k2 + k3) / 2, 48, true); // shift+click gray segment 2
  ...
  // hold S while playing: caret scans backward, resumes on release
  ...
  await new Promise((r) => setTimeout(r, 700)); // past 250ms hold threshold -> scanning
  ...
  // hold F while paused: scans forward, stays paused on release
  ...
  // [20] playhead follows - timeline pans to keep the caret in frame
  ...
  const farT = v0.start + v0.w + 0.5; // beyond the right edge
  ...
  const backT = v1.start - 0.5; // before the left edge
  ...
  // [21] firstKeptTime skips the whole adjacent gray run
  console.log('[21] firstKeptTime across a gray run');
  await js(`window.__app.model.reset(${DUR}); window.__app.model.snapshot(); window.__app.refreshActiveKeys();`);
  await js(`window.__app.timeline.selectedIndex = null; window.__app.seekTo(${k1}); window.__app.cut(); window.__app.seekTo(${k2}); window.__app.cut(); window.__app.seekTo(${k3}); window.__app.cut();`);
  await js(`window.__app.timeline.selectedIndex = 1; window.__app.deleteSegment();`); // gray [k1,k2]
  await js(`window.__app.timeline.selectedIndex = 2; window.__app.deleteSegment();`); // gray [k2,k3]
  const fkRun = await js(`window.__app.firstKeptTime(${k1})`);
  check('firstKeptTime jumps the whole gray run -> k3', near(fkRun, k3));

  // [22] no kept content eaten: guard fires only at/after the gray boundary
  ...
  // [23] overlapping skipTo must not leave the video muted
  console.log('[23] skipTo mute robustness');
  await js(`window.__app.video.muted = false; window.__app.cancelSkipMute();`);
  await js(`window.__app.skipTo(5, false);`);
  await js(`window.__app.skipTo(6, false);`); // second skip before 'seeked'
  ...
  // [24] transport: start/end + block navigation
  ...
  // [25] gray skip lands exactly on the kept start keyframe (the first sample of
  // the kept segment is heard; the boundary belongs to the segment starting
  // there, so the guard never re-triggers and skips kept audio).
  ...
  // [26] Alt+click multi-selects kept segments; X dims them all; one undo
  console.log('[26] Alt+click multi-select');
  await js(`window.__app.model.reset(${DUR}); window.__app.model.snapshot(); window.__app.refreshActiveKeys(); window.__app.timeline.selectedIndex = null;`);
  await js(`window.__app.timeline.selectedIndex = null; window.__app.seekTo(${k1}); window.__app.cut(); window.__app.seekTo(${k2}); window.__app.cut(); window.__app.seekTo(${k3}); window.__app.cut();`);
  await clickAt((k1 + k2) / 2, 48, false, true); // alt+click segment 1
  await clickAt((k2 + k3) / 2, 48, false, true); // alt+click segment 2
